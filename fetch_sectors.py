...
def fetch_sectors():
    ...
    # ── 來源2：旺德富/旺得富直接 RSS
    mtkstock_urls = [
        'https://news.mtkstock.com.tw/rss',
        'https://www.wantgoo.com/rss/stock-news',
        'https://www.wantgoo.com/rss',
    ]
    for rss_url in mtkstock_urls:
        try:
            feed = feedparser.parse(rss_url)
            print(f'旺德富 RSS {rss_url} → {len(feed.entries)} 筆')
            for entry in feed.entries:
                raw_title = entry.get('title', '')
                if '熱門族群' not in raw_title:
                    continue
                ...
                time_str = fmt_time(pub_ts)
                if title:
                    new_items.append({'title': title, 'time': time_str, 'url': link, 'src': '旺德富', 'ts': pub_ts})
                    print(f'  ✅ 旺德富: {title[:40]}')
        except Exception as e:
            print(f'旺德富 RSS error {rss_url}: {e}')
    try:
        r = requests.get(
...
def main():
    ...
    # 合併去重（用 title 做 key，避免同篇不同URL重複）
    item_map = {}
    for n in old_items:
        if '熱門族群' not in n.get('title', ''):
            continue
    ...
    print(f'合併後共 {len(merged)} 則（15天內，標題含熱門族群）')
